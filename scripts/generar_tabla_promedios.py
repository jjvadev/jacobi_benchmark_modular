#!/usr/bin/env python3
"""
Genera tabla resumen de promedios (wall_s) para todas las variantes Jacobi.
...
def write_summary_csv(data: dict, output_csv: Path) -> None:
    """Escribe tabla CSV con promedios por variante y n (k como referencia)."""
    ...
    # Ordenar implementaciones: seq, seq_mem, seq_o3, threads_2/4/8/16, processes_2/4/8/16
    def sort_key(impl):
        order = ["seq", "seq_mem", "seq_o3"]
    ...
    implementations = sorted(data.keys(), key=sort_key)
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for impl in implementations:
            ...
            for n in n_values:
                if n in data[impl]:
                    row.append(f"{data[impl][n]:.6f}")
                else:
                    row.append("N/A")
            writer.writerow(row)


def write_summary_png(csv_path: Path, output_png: Path) -> None:
    ...
    for row_idx in range(1, n_rows):
        ...
        label_cell.set_facecolor("#D9E2F3")
        
        if row_idx % 2 == 0:
            for col_idx in range(n_cols):
                table[(row_idx, col_idx)].set_facecolor("#F2F2F2")
    
    plt.title("Tabla resumen: Promedios de tiempo (segundos) por variante y N (k referencia)", 
...
def main():
    ...
    write_summary_csv(data, OUTPUT_CSV)
    
    print(f"Generando PNG: {OUTPUT_PNG}")
    write_summary_png(OUTPUT_CSV, OUTPUT_PNG)
    
    print("\nOK: Tablas de promedios generadas exitosamente")

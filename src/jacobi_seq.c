...
void jacobi_seq(JacobiContext *ctx) {
    ...
    for (sweep = 0; sweep < ctx->nsweeps; ++sweep) {
         ...
         *
         * Los bordes i=0 e i=n no se tocan aqui (Dirichlet fijas).
         *
         ...
         *
         * RMS = sqrt( sum(r_i^2) / (n+1) )
         * Se divide por (n+1) para normalizar por cantidad de nodos.
         */
        sweeps_done = sweep + 1;
